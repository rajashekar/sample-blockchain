...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            block.hash = SHA256(JSON.stringify(block)).toString(encHex);
    ...
    getStarsByWalletAddress (address) {
        ...
        let stars = [];
        return new Promise((resolve, reject) => {
           let allBlocks = self.chain.slice(1)
           allBlocks.map(block => block.getBData())
           .filter(p => p.then(plain_data => plain_data.address == address ? stars.push(plain_data.star) : ""))
